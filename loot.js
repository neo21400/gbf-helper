...
// Экранирование для вставки данных (имена предметов приходят от игры/импорта/сервера
// синхронизации) в innerHTML — без него возможна HTML-инъекция в страницу расширения
const esc = (s) => String(s).replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]));

// type содержит слэш (например "item/article"), поэтому encodeURIComponent не подходит —
// вместо этого вырезаем всё, кроме безопасных для пути символов
const gbfItem = (type, id) => `https://prd-game-a-granbluefantasy.akamaized.net/assets_en/img_mid/sp/assets/${String(type).replace(/[^\w\/-]/g, '')}/s/${String(id).replace(/[^\w.-]/g, '')}.jpg`;
...
const raidIDs = {
    ...
    /*Odious 18*/ 305721: "Nihuyvintae", 305711: "Belmerkvolk", 305731: "Narophirmidas", 305741: "Macutanmacar", "305751": "Papahlukruva", 305761: "Zamalvoch",
    /*Odious6 */ 305781: "Nihuyvintae",
...
window.onload = async (e) => {
    ...
    // проще, чем полагаться на кнопку. Результат придёт асинхронно через
    // chrome.storage.onChanged (см. ниже) и сам перерисует текущий рейд.
    ...
    document.getElementById("btn-favorites-tab").onclick = () => {
        chrome.storage.sync.set({ "lastTab": "favorites" });
        document.querySelectorAll(".raid-btn.active").forEach(b => b.classList.remove("active"));
        document.getElementById("btn-favorites-tab").classList.add("active");
        
        currentRaid = null;
        raidTitle.textContent = "Favorites";
        ...
        if (favoriteRaids.length === 0) {
            const noMsg = document.createElement("div");
            noMsg.id = "no-loot-msg";
            noMsg.textContent = "No favorite raids added.";
            raidInfo.appendChild(noMsg);
            return;
        }
        ...
        favoriteRaids.forEach(favId => {
            ...
            header.className = "favorite-raid-header";
            const displayName = customNames[favId] ? customNames[favId] : (raidIDs[favId] || favId);
            const totalKills = raidData[favId] ? Object.keys(raidData[favId]).filter(k => k !== 'name').length : 0;
            header.innerHTML = `<span>${esc(displayName)}</span><span style="color:#666;font-weight:400;">Kills: <span style="color:#0288d1">${esc(totalKills)}</span></span>`;
            card.appendChild(header);

            const body = document.createElement("div");
            body.className = "favorite-raid-body";
            
            if (!raidData[favId] || totalKills < 1) {
                const emptyMsg = document.createElement("div");
                emptyMsg.style.color = "#444";
                emptyMsg.style.fontSize = "12px";
                emptyMsg.textContent = "No data recorded.";
    ...
    if (soloFights.length > 0) {
        ...
        soloGroup.className = "cat-group";
        const soloBtn = document.createElement("button");
        soloBtn.className = "cat-btn raid-category-button";
        soloBtn.innerHTML = '<span class="arrow">▶</span> Solo Fights';
        const soloChildren = document.createElement("div");
        soloChildren.className = "cat-children";
        ...
        function buildSoloCategory(catName, fightDictionary) {
            ...
            if (filteredSoloFights.length === 0) return;
            const subBtn = document.createElement("button");
            subBtn.className = "sub-cat-btn raid-category-button";
            subBtn.innerHTML = `<span class="arrow">▶</span> ${catName}`;
            const subDiv = document.createElement("div");
            subDiv.className = "sub-cat-children";
            Object.keys(fightDictionary).forEach(z => {
                Object.keys(fightDictionary[z]).forEach(f => {
                    if (filteredSoloFights.includes(parseInt(f))) {
                        ...
                        navButton.className = "raid-btn raid-button solo-quest";
                        ...
                        subDiv.appendChild(navButton);
                    }
                });
            });
            soloChildren.appendChild(subBtn);
            soloChildren.appendChild(subDiv);
        }
        buildSoloCategory("Arcarum", arcarumFights);
        buildSoloCategory("Free Quests", freeQuestFights);
        buildSoloCategory("Special Quests", specialQuestFights);
        buildSoloCategory("Co-op Quests", coopFights);
        ...
        soloFights.forEach(fight => {
            ...
            soloChildren.appendChild(navButton);
        });
        soloGroup.appendChild(soloBtn);
        soloGroup.appendChild(soloChildren);
        sidebar.appendChild(soloGroup);
    }

    // Category dropdown toggles
    document.querySelectorAll(".cat-btn.raid-category-button").forEach(btn => {
        btn.onclick = () => {
    ...
    document.querySelectorAll(".sub-cat-btn.raid-category-button").forEach(btn => {
        btn.onclick = () => {
            const children = btn.nextElementSibling;
            btn.classList.toggle("open");
            children.classList.toggle("visible");
    ...
    let lastRaidId;
    let isFavoritesLast = false;
    await chrome.storage.sync.get("lastTab").then(r => {
        if (r.lastTab === "favorites") {
            isFavoritesLast = true;
        } else {
            lastRaidId = parseInt(r.lastTab);
        }
    });
    if (!isFavoritesLast && !knownRaidIDs.includes(lastRaidId)) lastRaidId = 301061;
    ...
    document.querySelectorAll(".raid-button").forEach((i) => {
        ...
        i.textContent = customNames[raidID] ? customNames[raidID] : raidIDs[raidID];
        i.onclick = () => {
            document.querySelectorAll(".raid-btn.active").forEach(b => b.classList.remove("active"));
            document.getElementById("btn-favorites-tab").classList.remove("active");
            i.classList.add("active");
            chrome.storage.sync.set({ "lastTab": raidID });
            currentRaid = raidID;
            currentRaidName = i.textContent;
            raidTitle.textContent = i.textContent;
            ...
            if (favoriteRaids.includes(parseInt(raidID))) {
                btnFav.className = "fa fa-star";
                btnFav.classList.add("active");
            } else {
                btnFav.className = "fa fa-star-o";
                btnFav.classList.remove("active");
            }

            btnFav.onclick = () => {
                const rId = parseInt(currentRaid);
                if (favoriteRaids.includes(rId)) {
                    favoriteRaids = favoriteRaids.filter(id => id !== rId);
                    btnFav.className = "fa fa-star-o";
                    btnFav.classList.remove("active");
                } else {
                    favoriteRaids.push(rId);
                    btnFav.className = "fa fa-star";
                    btnFav.classList.add("active");
                }
                chrome.storage.sync.set({ "favoriteRaids": favoriteRaids });
            };
            renameBtn.onclick = () => {
                const name = prompt("Enter a new name for this fight", i.textContent);
                if (!name || name.trim() === "") {
            ...
            if (!raidData[raidID]) {
                ...
                buildRaidInfo({ ...raidData[raidID], name: i.textContent });
            }
        };
        if (raidID == lastRaidId) lastRaidBtn = i;
    });

    // Wire up always-visible action buttons
    document.getElementById("download-button").onclick = download;
    document.getElementById("upload-button").onclick = () => document.getElementById("upload-file").click();
    document.getElementById("upload-file").addEventListener("change", (e) => {
        ...
        reader.onload = (ev) => {
            try {
                ...
                // Импорт возможен только когда выбран конкретный рейд (числовой ID),
                // иначе данные записались бы под мусорным ключом хранилища
                if (!rId || !/^\d+$/.test(String(rId))) { alert("Select a raid first"); return; }
                if (!raidData[rId]) raidData[rId] = {};
                Object.assign(raidData[rId], data);
                chrome.storage.local.set({ [rId]: raidData[rId] }).then(() => window.location.reload());
    ...
    function buildRaidInfo(result, container = raidInfo, rId = getCurrentRaid(), isFav = false) {
        delete result.name;
        if (!isFav) {
            container.innerHTML = "";
            pinRow1.innerHTML = "";
    ...
    function processArtifacts(artifacts, container, isFav) {
        ...
        const body = document.createElement("div");
        body.className = "loot-section-body";

        for (let element in artis) {
            const row = document.createElement("div");
            row.className = "item-row";
            const img = gbfElement(element);
            const eleCount = Object.keys(artis[element]).map(i => artis[element][i]).reduce((acc, n) => acc + n);
            row.innerHTML = `<img class="item-img" src="${esc(img || '')}" style="width:40px;height:auto;">
                <span class="item-name">${esc(element)}</span>
                <span class="item-count">x${esc(eleCount)}</span>
                <span class="item-pct">${(eleCount / count * 100).toFixed(1)}%</span>`;
    ...
    function pinItemContextMenu(e, item) {
        ...
        pinContextMenu.style.top = `${e.clientY}px`;
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
        ...
        for (let k in changes) {
            if (/^\d+$/.test(k)) {
                raidData[k] = changes[k].newValue || {};
                if (favoriteRaids.includes(parseInt(k))) {
        ...
        if (currentRaidId && changes[currentRaidId]) {
            ...
            // Re-trigger the favorites tab logic to rebuild the view
            document.getElementById("btn-favorites-tab").dispatchEvent(new Event("click"));
        }
    });
}
